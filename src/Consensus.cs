...
{
    private Logger _logger = Logger.GetLogger<Consensus>();

    private ImmutableList<OnionRouter> _parsed = ImmutableList<OnionRouter>.Empty;
    ...
    public async Task ParseAsync(Stream content, CancellationToken cancellationToken)
    {
        ...
        while(!reader.EndOfStream && _parsed.Count < 200)
        {
            ...
            else if(line.StartsWith("s "))
            {
                ...
                {
                    _parsed = _parsed.Add(onionRouter);
                }
            }
        }
    }

    public ImmutableList<OnionRouter> OnionRouters
        => _parsed;

    public ImmutableList<OnionRouter> GuardRelays
        => OnionRouters.Where(o=>o.Flags.HasFlag(StatusEntryS.Guard)).ToImmutableList();
}